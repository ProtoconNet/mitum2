...
func (t *testNewOperationPool) TestTraverseOperationsBytes() {
	...
	t.Run("offset", func() {
		...
		t.NoError(pst.TraverseOperationsBytes(context.Background(), nil, func(_ string, _ FrameHeaderPoolOperation, _, offset []byte) (bool, error) {
			defer func() {
				i++
			}()
		...
		t.NoError(pst.TraverseOperationsBytes(context.Background(), half, func(_ string, meta FrameHeaderPoolOperation, body, offset []byte) (bool, error) {
			op := ops[i]
	...
	t.Run("invalid offset", func() {
		...
		t.NoError(pst.TraverseOperationsBytes(context.Background(), invalid, func(_ string, _ FrameHeaderPoolOperation, _, offset []byte) (bool, error) {
			i++
	...
	t.Run("callback error", func() {
		...
		err := pst.TraverseOperationsBytes(context.Background(), nil, func(_ string, meta FrameHeaderPoolOperation, body, offset []byte) (bool, error) {
			if i >= len(ops)/2 {
